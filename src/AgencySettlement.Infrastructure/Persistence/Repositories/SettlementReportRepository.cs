...
namespace AgencySettlement.Infrastructure.Persistence.Repositories
{
    ...
    CancellationToken cancellationToken)
        ...
        private async Task<Dictionary<long, decimal>> GetSettlementPaymentAmountsAsync(
    List<long> settlementIds,
    CancellationToken cancellationToken)
        {
            return await _context.SettlementPayments
                .AsNoTracking()
    ...
    CancellationToken cancellationToken)
        {
            ...
            var plan5Quota = await _context.SettlementOrderItems
                .AsNoTracking()
                .Where(x =>
                    x.AgencyId == agencyId &&
                    x.YearId == yearId &&
            ...
            var contractFloorAmount = await _context.SettlementOrderItems
                .AsNoTracking()
                .Where(x =>
                    x.AgencyId == agencyId &&
                    x.YearId == yearId &&
            ...
            return new SettlementDetailsDto
            {
                ...
                TotalStudentOnlineSite = onlineItems
                    .Where(x =>
                        x.RegistrationPlanId == 8 &&
                        x.ExamModeId == 1)
                    .Sum(x => x.CandidateCount),
